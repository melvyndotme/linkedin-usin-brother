...
import { generateBrotherWaveCorporateSVG, generateBrotherWebsiteBannerSVG } from '../lib/svgBrotherWebsiteTemplates.js';
...
export default function Module2AIPosts({ isDark }) {
  ...
  const waveSvg = generateBrotherWaveCorporateSVG({
    ...
    headline: selectedNews.title.length > 40 ? selectedNews.title.slice(0, 40) + "..." : selectedNews.title,
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className={`p-6 rounded-2xl border transition-colors ${
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-md bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 text-xs font-bold uppercase tracking-wider mb-2">
              <Sparkles className="w-3.5 h-3.5" />
              Module 2: Employer Branding & AI Intelligence
            </div>
            <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
              AI Innovation & Employee Productivity Engine
            </h2>
            <p className="text-xs text-slate-500 mt-1 max-w-2xl">
              Translates the latest 24h AI technology breakthroughs into our 3-pillar employer branding narrative:
              <strong className="text-[#0f2ea2]"> What it is → Why it matters → Breakthrough productivity for Brother Singapore employees</strong>.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between text-[10px] text-slate-500 mb-1">
                      <span className="font-semibold text-[#0f2ea2]">{item.source}</span>
                      <span>{item.date}</span>
                    </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className={`p-6 rounded-2xl border space-y-5 ${
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b pb-4 dark:border-slate-800">
              <div>
                <span className="text-[11px] font-mono text-[#0f2ea2] font-bold uppercase tracking-wider">
                  Employer Branding Multi-Draft Engine
                </span>
                <h3 className={`text-base font-bold mt-0.5 line-clamp-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {currentDraft.angle}
                </h3>
              </div>
            ...
            <div className="bg-cyan-50 dark:bg-cyan-950/40 border border-cyan-200 dark:border-cyan-500/30 rounded-xl p-3.5">
            ...
            <div className="space-y-2 pt-2">
              <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  Paired Brother Corporate Graphic (Toner Wave Style)
                </h4>

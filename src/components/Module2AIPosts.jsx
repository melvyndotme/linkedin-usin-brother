...
export default function Module2AIPosts({ isDark, onNavigateToDraftStudio }) {
  ...
  const handleTriggerNotionAI = async () => {
    ...
    try {
      ...
      setNotionStatus('2/2 Notion page created! Notion Custom Agents are generating drafts in Notion...');

      let attempts = 0;
      const interval = setInterval(async () => {
        attempts++;
        try {
          const pageRes = await fetch(`/api/notion/page?pageId=${pageId}&apiKey=${encodeURIComponent(token)}`);
          const pageData = await pageRes.json();

          if (pageData.success && (pageData.draft1 || pageData.draft2)) {
            setLiveNotionNewsDrafts(prev => ({
              ...prev,
              [newsKey]: {
                draft1: pageData.draft1 || prev[newsKey]?.draft1 || '',
                draft2: pageData.draft2 || prev[newsKey]?.draft2 || '',
                rationale: pageData.rationale || prev[newsKey]?.rationale || '',
                url: pageData.url || data.url
              }
            }));

            // If both draft1 and draft2 (or attempts >= 12) are ready, stop polling
            if ((pageData.draft1 && pageData.draft2) || attempts >= 12) {
              clearInterval(interval);
              setNotionSyncing(false);
              setNotionStatus('✅ Live drafts generated by Notion AI & reflected below!');
              return;
            } else {
              setNotionStatus(`Draft 1 received! Waiting for queued Kaizen draft in Notion (attempt ${attempts}/12)...`);
            }
          }
          ...
          if (attempts >= 12) {
            clearInterval(interval);
            setNotionSyncing(false);
            setNotionStatus('Post created in Notion! Custom Agents are queued. Click "Open in Notion" or "Fetch Latest" below.');
          } else if (!pageData.draft1) {
            setNotionStatus(`2/2 Notion Custom Agents writing in Notion (attempt ${attempts}/12)...`);
          }
        } catch (pollErr) {

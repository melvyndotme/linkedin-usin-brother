...
import { Calendar, Sparkles, Copy, Check, Download, Eye, ArrowRight, ShieldCheck, Flame, Layers, ExternalLink, RefreshCw, AlertCircle, Database } from 'lucide-react';
...
export default function Module1EventPosts({ isDark }) {
  ...
  // Notion AI Live Generation & Reflection State
  const [notionSyncing, setNotionSyncing] = useState(false);
  const [notionStatus, setNotionStatus] = useState('');
  const [notionPageUrl, setNotionPageUrl] = useState(null);
  const [liveNotionDrafts, setLiveNotionDrafts] = useState({});
  ...
  const handleTriggerNotionAI = async () => {
    ...
    setNotionStatus('1/2 Creating post in Notion with Source Context...');
    ...
    try {
      const res = await fetch('/api/notion/sync', {
        ...
        body: JSON.stringify({
          ...
          databaseId: dbId,
          post: {
            title: `${selectedOccasion.name} 2026`,
      ...
      setNotionStatus('✅ 100% Autonomous: Post & Drafts populated in Notion! Click "Open in Notion" to view.');
    } catch (err) {
      setNotionSyncing(false);
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-slate-600 dark:text-slate-400">
                    {notionStatus || "Sends MOM occasion to Notion. Notion Custom Agents generate Draft 1 & 2 in real-time."}
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b pb-3.5 dark:border-slate-800">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {currentDraft.isLiveNotion && (
                    <span className="text-[10px] px-2 py-0.2 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold border border-emerald-500/20">
                      ⚡ Notion Custom Agent
                    </span>
                  )}
                </div>
